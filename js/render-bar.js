...
// generate() whenever it changes, so this runs very often - keep it cheap.
      ...
      function generate() {
        ...
        if (showIndicatorKey.checked) {
          ...
          const keySizeVal = clampField(indicatorKeySize, 6, 50, 25);
...
`;
        ...
        // Instant, cheap estimate of the export height so the field never
        // feels laggy while dragging a slider or typing. This can be a
        // few pixels off from the real export (see scheduleExportHeightRefresh
        // in download-image.js), which corrects it shortly after.
        if (exportArea.offsetWidth && exportArea.offsetHeight) {
          aspect = exportArea.offsetWidth / exportArea.offsetHeight;
          if (lockRatio.checked) {
            exportHeight.value = Math.round((parseInt(exportWidth.value) || 3840) / aspect);
          }
        }
        if (typeof scheduleExportHeightRefresh === 'function') {
          scheduleExportHeightRefresh();
        }
      }
