...
// read as one continuous segment after another.
      ...
      function addRow(scrollToRow = true) {
        ...
        // in percentage mode, neither field can go past 100%; in pages mode
        // they're capped at the book's total page count (if known)
        const rowMax = trackingMode === 'pages' ? (getTotalPages() || '') : 100;
        let r = document.createElement('div');
        r.className = 'row';
        r.innerHTML = `
<button type="button" class="drag-handle" title="Drag to reorder" aria-label="Drag to reorder segment">⠿</button>

<div class="field">
<label class="row-label-from">${rowLabelText('From')}</label>
<input type="number" min="0" max="${rowMax}" value="${defaultFrom}" oninput="clampSegmentInput(this);generate()">
</div>

<div class="field">
<label class="row-label-to">${rowLabelText('To')}</label>
<input type="number" min="0" max="${rowMax}" value="${defaultTo}" oninput="clampSegmentInput(this);generate()">
</div>
...
`;
      ...
      // Keeps a From/To field from exceeding its max (100% in percentage
      // mode, or the book's total pages in page mode).
      function clampSegmentInput(input) {
        if (input.max === '') return;
      ...
      function focusNextToInput(currentRow) {
        const allRows = Array.from(rows.querySelectorAll('.row'));
        const idx = allRows.indexOf(currentRow);
        const nextRow = allRows[idx + 1];
        let targetRow = nextRow;
        if (!targetRow) {
          // the next row doesn't exist yet - create it, carrying the
          // current "To" value forward as its "From" (same as Tab/blur does)
          const toVal = currentRow.querySelectorAll('input[type=number]')[1].value;
          addRow();
        ...
        const targetTo = targetRow.querySelectorAll('input[type=number]')[1];
        targetTo.focus();
        targetTo.select();
      }
      ...
      function updateRowLabels() {
        ...
        document.querySelectorAll('.row input[type=number]').forEach(inp => {
          inp.max = max;
          clampSegmentInput(inp);
        });
      }
