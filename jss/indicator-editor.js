...
// this same list.
      ...
      function drawPalette() {
        ...
        indicators.forEach(m => {
          ...
          d.dataset.id = m.id;
          d.innerHTML = `
<button type="button" class="drag-handle" title="Drag to reorder" aria-label="Drag to reorder indicator">⠿</button>
<custom-color-picker
value="${m.color}"
...
`;
      ...
      // After the user drags a swatch row to a new spot, re-sorts the
      // indicators array to match the new DOM order so dropdowns, the
      // indicator key, and saved files all reflect the new sequence.
      function reorderIndicatorsFromDOM() {
        const domIds = Array.from(palette.querySelectorAll('.swatch-row')).map(row => row.dataset.id);
        indicators.sort((a, b) => domIds.indexOf(a.id) - domIds.indexOf(b.id));
